...
async function main() {
    ...
    // Finansal statülü ama stok_hareketleri olmayan faturaları bul (sadece kalem içerenler)
    ...
    for (const fatura of invoices) {
        if (fatura.kalemler.length === 0) continue;

        const isSatis = fatura.faturaTipi === 'SATIS';
        const hasMissingStock = fatura.kalemler.some(k => k.stokHareketler.length === 0);
        const hasMissingCari = fatura.cariHareketler.length === 0;
        ...
        if (hasMissingStock || hasMissingCari) {
            ...
            await prisma.$transaction(async (tx) => {
                ...
                // Stok Hareketleri Eksikse Tamamla
                if (hasMissingStock) {
                    for (const kalem of fatura.kalemler) {
                        if (kalem.stokHareketler.length === 0) {
